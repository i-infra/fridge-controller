...
class Fridge:
    ...
    def set_compressor(self):
        minute = self.last.get("timestamp")[-3]
        if self.last.get(self.GRATE_SENSOR):
            for minute_range in self.temp:
                if minute in range(*minute_range):
                    if self.last.get(self.GRATE_SENSOR) < self.temp.get(minute_range):
                        self.compressor.off()
                    else:
                        self.compressor.on()
